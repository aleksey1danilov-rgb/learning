...
@router.get("/request-status", response_class=HTMLResponse)
async def request_status_page(request: Request):
    """Страница статуса заявки"""
    return templates.TemplateResponse("request_status.html", {"request": request})
...
@router.get("/curses", response_class=HTMLResponse)
async def curses_page(request: Request):
    return templates.TemplateResponse("curses.html", {"request": request})
...
async def test_registration_check(
    ...
    if user.registration_status == "pending":
        ...
        <a href="/pages/complete-registration">Complete registration</a>
        """)
    return HTMLResponse(f"""
    <h1>Registration completed</h1>
    <p>Welcome, {user.full_name or user.username}!</p>
    <p>Status: {user.registration_status}</p>
    """)
...
@router.get("/admin/status-logs", response_class=HTMLResponse)
async def admin_status_logs(request: Request):
    return templates.TemplateResponse("admin_status_logs.html", {"request": request})
